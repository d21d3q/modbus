...
func TestTCPServerUnknownFunctionAndBounds(t *testing.T) {
	server, addr := startTCPServerForRaw(t, &tcpTestHandler{})
	defer server.Stop()
	...
	// Unknown function codes should map to illegal function exceptions.
	res, err := sendRawMBAPRequest(conn, 0x09, 0x45, nil)
	if err != nil {
		t.Fatalf("unexpected unknown-function error: %v", err)
	}
	if res.functionCode != (0x80|0x45) || len(res.payload) != 1 || res.payload[0] != exIllegalFunction {
		t.Fatalf("unexpected unknown-function response: %#v", res)
	}

	// Address wraparound should map to illegal data address.
	res, err = sendRawMBAPRequest(conn, 0x09, fcReadHoldingRegisters, []byte{0xff, 0xff, 0x00, 0x02})
	if err != nil {
		t.Fatalf("unexpected bounds-check error: %v", err)
	}
	if res.functionCode != (0x80|fcReadHoldingRegisters) || len(res.payload) != 1 || res.payload[0] != exIllegalDataAddress {
		t.Fatalf("unexpected bounds response: %#v", res)
	}
}

func TestTCPServerProtocolErrorClosesConnection(t *testing.T) {
	server, addr := startTCPServerForRaw(t, &tcpTestHandler{})
	defer server.Stop()
	...
	// Invalid single-coil payload (value must be 0xff00 or 0x0000) is protocol error.
	_, err = sendRawMBAPRequest(conn, 0x09, fcWriteSingleCoil, []byte{0x00, 0x01, 0x12, 0x34})
	if err == nil {
		t.Fatalf("expected the server to close connection on protocol error")
	}
	if isTimeoutError(err) {
		t.Fatalf("expected disconnect, got timeout: %v", err)
	}
...
func TestTCPServerHandlerLengthMismatchReturnsServerFailure(t *testing.T) {
	server, addr := startTCPServerForRaw(t, &mismatchHandler{})
	defer server.Stop()
	...
	if _, err = client.ReadCoils(0x0000, 2); err != ErrServerDeviceFailure {
		t.Fatalf("expected ErrServerDeviceFailure for coils, got: %v", err)
	}
	if _, err = client.ReadRegisters(0x0000, 2, HOLDING_REGISTER); err != ErrServerDeviceFailure {
		t.Fatalf("expected ErrServerDeviceFailure for registers, got: %v", err)
	}
}

func sendRawMBAPRequest(conn net.Conn, unitId byte, functionCode byte, payload []byte) (res *pdu, err error) {
	tt := newTCPTransport(conn, time.Second, nil)
	tt.lastTxnId = 7
	frame := tt.assembleMBAPFrame(tt.lastTxnId, &pdu{
		unitId:       unitId,
...
func startTCPServerForRaw(t *testing.T, handler RequestHandler) (server *ModbusServer, addr string) {
	t.Helper()
...
func (h *mismatchHandler) HandleCoils(req *CoilsRequest) (res []bool, err error) {
	if req.IsWrite {
...
func (h *mismatchHandler) HandleDiscreteInputs(req *DiscreteInputsRequest) (res []bool, err error) {
	return []bool{true}, nil
}

func (h *mismatchHandler) HandleHoldingRegisters(req *HoldingRegistersRequest) (res []uint16, err error) {
	if req.IsWrite {
		return []uint16{0x1}, nil
	}
	return []uint16{0x1}, nil
}

func (h *mismatchHandler) HandleInputRegisters(req *InputRegistersRequest) (res []uint16, err error) {
	return []uint16{0x1}, nil
}
