...
type ServerConfiguration struct {
	...
	URL string
	// Speed sets the serial link speed (rtu/ascii only)
	Speed uint
	...
	Logger *log.Logger
	// serialLinkFactory allows tests to inject custom serial links.
	serialLinkFactory func(*serialPortConfig) (rtuLink, error)
}
...
func (ms *ModbusServer) Stop() (err error) {
	...
	} else if ms.serialTransport != nil {
		err = ms.serialTransport.Close()
		ms.serialTransport = nil
		ms.serialLink = nil
	}
...
func (ms *ModbusServer) startSerialServer() (err error) {
	var link rtuLink
	var conf = &serialPortConfig{
		Device:   ms.conf.URL,
		Speed:    ms.conf.Speed,
	...
	if ms.conf.serialLinkFactory != nil {
		link, err = ms.conf.serialLinkFactory(conf)
		if err != nil {
			return
		}
	} else {
	...
	if ms.transportType == modbusRTU {
		ms.serialTransport = newRTUTransport(
			link, ms.conf.URL, ms.conf.Speed, ms.conf.Timeout, ms.conf.Logger)
	} else {
		discard(link)
	...
	go ms.handleSerialTransport(ms.serialTransport)

	return
}

func (ms *ModbusServer) handleSerialTransport(t transport) {
	ms.handleTransport(t, ms.conf.URL, "")

	ms.lock.Lock()
	defer ms.lock.Unlock()

	if ms.serialTransport == t {
		ms.serialTransport = nil
		ms.serialLink = nil
		ms.started = false
	}
...
func (ms *ModbusServer) handleTransport(t transport, clientAddr string, clientRole string) {
	...
	for {
		...
		if err != nil {
			...
			if (ms.transportType == modbusRTU || ms.transportType == modbusASCII) &&
				(err == ErrBadCRC || err == ErrBadLRC ||
					err == ErrShortFrame || err == ErrProtocolError) {
				ms.logger.Warningf(
					"ignoring malformed serial frame (client address: '%s'): %v",
					clientAddr, err)
				continue
			}
