...
func (rt *rtuTransport) ReadRequest() (req *pdu, err error) {
	...
	n, err = io.ReadFull(rt.link, header[:])
	if (n > 0 || err == nil) && n != len(header) {
		err = ErrShortFrame
		return
	}
	if err != nil {
		if os.IsTimeout(err) || err == ErrRequestTimedOut {
	...
	case fcReadHoldingRegisters,
		...
		fcWriteSingleCoil:
		payload = make([]byte, 4)
		n, err = io.ReadFull(rt.link, payload)
	...
	case fcWriteMultipleRegisters, fcWriteMultipleCoils:
		prefix := make([]byte, 5)
		n, err = io.ReadFull(rt.link, prefix)
		if err != nil && err != io.ErrUnexpectedEOF {
			return
		}
		if n != len(prefix) {
		...
		if byteCount > 0 {
			...
			n, err = io.ReadFull(rt.link, data)
			if err != nil && err != io.ErrUnexpectedEOF {
				return
			}
			if n != len(data) {
	...
	default:
		payload, crcBytes, err = rt.readUnknownRequestBody()
		if err != nil {
			return
		}
	}
	...
	if crcBytes == nil {
		...
		n, err = io.ReadFull(rt.link, crcBytes)
		if err != nil && err != io.ErrUnexpectedEOF {
			return
		}
		if n != len(crcBytes) {
	...
	if len(payload)+2+2 > maxRTUFrameLength {
...
func (rt *rtuTransport) readUnknownRequestBody() (payload []byte, crcBytes []byte, err error) {
	var tmp [1]byte
	var n int
	var body []byte

	for {
		n, err = rt.link.Read(tmp[:])
		if err != nil {
			if err == ErrRequestTimedOut || os.IsTimeout(err) {
				...
				payload = append(payload, body[:len(body)-2]...)
				crcBytes = []byte{body[len(body)-2], body[len(body)-1]}
				err = nil
				return
			}
			return
		...
		body = append(body, tmp[0])
		if len(body)+2 > maxRTUFrameLength {
			err = ErrProtocolError
			return
		}
	}
